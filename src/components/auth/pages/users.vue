    ...
    <el-row>
      <el-col :sm="24" :md="24" :lg="20">
        <el-card class="box-card">
          ...
          <el-button icon="plus" @click="addUserData.visible = true" style="margin-bottom: 12px">新建用户</el-button>
          <el-table
            :data="userList"
            border>
            <el-table-column
            ...
            <el-table-column
              ...
              <template scope="scope">
                <el-button type="info" :plain="true" size="small" icon="edit" @click="editUserData.visible = true; editUserData.user = scope.row"></el-button>
                <el-button type="danger" size="small" icon="delete" @click="onDeleteUser(scope.row)"></el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
  ...
  export default {
    ...
    methods: {
      ...
      onAddUser ({ code, userId, email, level }) {
        if (!/^\w+$/.test(userId)) {
          this.$message.error('用户 ID 只可包含英文、数字和下划线！')
          return
        }
        let postData = {
          action: 'permission/users',
      ...
      onEditUser ({ code, userId, email, level }) {
        if (!/^\w+$/.test(userId)) {
          this.$message.error('用户 ID 只可包含英文、数字和下划线！')
          return
        }
      ...
      onDeleteUser ({ code, userId }) {
        this.$confirm(`此操作将移除用户「${code}」，是否继续？`, '提示', {
